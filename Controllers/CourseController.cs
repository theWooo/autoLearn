...
namespace diplom.Controllers {
    public class CourseController : Controller {
        ...
        public async Task<IActionResult> CourseWorkshop() {
            ...
            CourseBatchDTO batch = new CourseBatchDTO();
            SqlDataReader reader = await DI.getDiContainer().asyncExecuteReader(@$"select courseName,courseDescription,courseImageLink, id from course where creatoridfk = {DI.getDiContainer().getTokenValues(token).First(it => it.Type == "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/authentication").Value}");//find all of all the courses that the current user has created by id
            ...
            while (reader.Read()) {
                batch.courses.Add(new Course { courseDescription=reader.GetValue(1) as string, courseImageLink=reader.GetValue(2) as string, courseName=reader.GetValue(0) as string,courseId = (int)reader.GetValue(3) });
            }
            return View(batch);
        }
        ...
        public IActionResult SubmitCourseChanges() {
            return RedirectToAction("Course", "CourseWorkshop");
        }
    }
}
